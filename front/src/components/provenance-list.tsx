      ...
      {items.map((p) => (
        <li key={p.id} className="flex items-center justify-between gap-3 px-3 py-2">
          <div className="min-w-0">
            ...
            <span className="ml-2 truncate text-muted-foreground">{p.value || "(ausente)"}</span>
          </div>
          <div className="flex shrink-0 items-center gap-2">
            ...
            <span className="hidden text-xs text-muted-foreground sm:inline">{fmtRelative(p.found_at)}</span>
          </div>
        </li>
      ))}
    </ul>
